...
class GAT_Fourier(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads, adj, num_nodes = 247, hidden_size_factor=1, embed_size=32, sparsity_threshold=0.01, levels=3, pre_L=1):
        super(GAT_Fourier, self).__init__()
        self.adj = adj
        self.nfeat = nfeat
        ...
        self.attentions = nn.ModuleList([
            FreqGraphAttentionLayer(nfeat, nfeat, dropout, alpha) 
        ...
        '''self.multiscale_fuser = MultiScaleFusion(
            nfeat=nfeat,
            levels=levels  
        )'''
    ...
    def FGCN(self, x):
        ...
        '''fused_x = self.multiscale_fuser(x) #[B,N,F]
        enhanced_x = fused_x + x *0.5
        x = torch.fft.rfft(enhanced_x, dim=-1, norm="ortho")'''
        x = torch.fft.fft(x, dim=-1, norm="ortho")

        # FourierGNN
        # x = self.fourierGC(x, B, (N*L)//2 + 1, self.frequency_size)
        # x = self.freq_attention(x, B, N, L)
        ...
        # ifft
        x = torch.fft.ifft(x, n=self.nfeat, dim=2, norm="ortho").real
        return x + res
...
class FreqGraphAttentionLayer(nn.Module):
    def __init__(self, in_features, out_features, dropout, alpha, 
                 concat=True, num_nodes=247):
        super(FreqGraphAttentionLayer, self).__init__()
        self.dropout = dropout
...
class MultiScaleFusion(nn.Module):
    def __init__(self, nfeat, levels=3):
        super().__init__()
        self.levels = levels
        self.nfeat = nfeat
        self.wavelet_dec = nn.ModuleList([
            nn.Conv1d(nfeat, nfeat // (2**i), kernel_size=5, padding=2)
            for i in range(levels)
        ])

        # Projection layers to match reduced dimensions
        self.residual_proj = nn.ModuleList([
            nn.Linear(nfeat, nfeat // (2**i))
            for i in range(levels)
        ])

        # Projection layers to align feature dimensions
        self.align_proj = nn.ModuleList([
            nn.Linear(nfeat // (2**i), nfeat)
            for i in range(levels)
        ])

        # Calculate total_features correctly
        self.total_features = sum([nfeat // (2**i) for i in range(levels)]) + sum([nfeat // (2**i) for i in range(levels)])
        self.adaptive_proj = nn.Linear(self.total_features, levels + 1)

    def forward(self, x):  # x: [B, N, F]
        scales = []
        residual = x
        for i, conv in enumerate(self.wavelet_dec):
            x_conv = F.relu(conv(x.transpose(1, 2)))  # [B, F//(2**i), N]
            x_conv = x_conv.transpose(1, 2)  # [B, N, F//(2**i)]
            scales.append(x_conv)

        # Project residual to match each scale's feature dimension
        projected_residuals = [proj(residual) for proj in self.residual_proj]

        # Concatenate all features for adaptive projection
        fused_feats = torch.cat(scales + projected_residuals, dim=-1)
        logits = self.adaptive_proj(fused_feats.mean(dim=1))

        # Compute filter weights using Gumbel-Softmax
        filter_weights = F.gumbel_softmax(logits, tau=0.5, hard=False, dim=-1)

        # Reshape filter_weights for broadcasting
        filter_weights = filter_weights.unsqueeze(1).unsqueeze(2)  # [B, 1, 1, levels+1]

        # Align feature dimensions of all tensors to nfeat
        aligned_scales = [self.align_proj[i](scale) for i, scale in enumerate(scales)]
        aligned_residuals = [self.align_proj[i](res) for i, res in enumerate(projected_residuals)]

        # Apply weights to aligned scales and residuals
        weighted_scales = [w * f for w, f in zip(filter_weights.unbind(dim=-1), aligned_scales + aligned_residuals)]
        output = sum(weighted_scales)
