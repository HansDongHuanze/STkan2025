...
class WaveletGAT(nn.Module):
    ...
    def atten_com(self, x):
        ...
        for att in self.attentions:
            ...
            att_output = self.activate(att_output)
    ...
    def wavelet_time_fusion(self, x, wave, B, N, L):
        input = torch.stack([x, wave], dim=-1).squeeze()
        input = torch.reshape(input, (B, 2, N, L))
        input = self.freq_conv(input)
        input = torch.reshape(input, (B, N, L))
        input = self.activate(input)
    ...
    def wavelet_transform_along_last_dim(self, tensor: torch.Tensor, wavelet: str = 'db1', level: int = 1) -> torch.Tensor:
        """
        对输入张量的最后一个维度进行小波变换（优化版）。
        
        Args:
            tensor: 输入张量，形状为 (..., C)，其中 C 是通道数。
            wavelet: 小波类型，如 'db1', 'haar', 'sym5' 等。
            level: 小波分解的层级。
        
        Returns:
            变换后的张量，形状为 (..., K)，其中 K 是小波系数的总长度。
        """
        # 获取输入形状
        ...
        # 计算小波系数结构（只需一次）
        dummy_data = np.zeros(C)
        coeffs = pywt.wavedec(dummy_data, wavelet, level=level)
        coeff_slices = [slice(None)] * len(tensor.shape[:-1])
        
        # 使用PyTorch的unfold进行批量处理（如果level=1且小波类型支持）
        try:
            # 尝试向量化实现（适用于特定小波类型）
            pad_size = pywt.Wavelet(wavelet).dec_len - 1
            padded = torch.nn.functional.pad(tensor, (0, pad_size))
            transformed = pywt.wavedec(padded.numpy(), wavelet, level=level, axis=-1)
            transformed_tensor = torch.from_numpy(np.concatenate(transformed, axis=-1))
            return transformed_tensor[(*coeff_slices, slice(None, sum(c.size for c in coeffs)))]
            ...
            # 预分配内存
            transformed_flat = np.zeros((tensor_flat.shape[0], total_coeffs))
            
            # 使用列表推导式+并行处理（如果数据量大）
            if tensor_flat.shape[0] > 1000:
                from concurrent.futures import ThreadPoolExecutor
                with ThreadPoolExecutor() as executor:
                    results = list(executor.map(
                        lambda x: np.concatenate(pywt.wavedec(x, wavelet, level=level)),
                        tensor_flat.numpy()
                    ))
                transformed_flat = np.stack(results)
            else:
                transformed_flat = np.stack([
                    np.concatenate(pywt.wavedec(x, wavelet, level=level))
                    for x in tensor_flat.numpy()
                ])
            
            transformed_tensor = torch.from_numpy(transformed_flat)
            return transformed_tensor.reshape(*original_shape[:-1], total_coeffs)
