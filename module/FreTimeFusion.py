...
class CoupFourGAT(nn.Module):
    ...
    def freq_time_fusion(self, x, freq, B, N, L):
        input_1 = torch.stack([x, freq.real], dim=-1).squeeze()
        input_1 = torch.reshape(input_1, (B, 2, N, L))
        input_1 = self.freq_time_conv1(input_1)
        input_1 = self.sigmoid(input_1)
        input_2 = torch.stack([x, freq.imag], dim=-1).squeeze()
        input_2 = torch.reshape(input_2, (B, 2, N, L))
        input_2 = self.freq_time_conv2(input_2)
        input_2 = self.sigmoid(input_2)
